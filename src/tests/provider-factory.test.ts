...
describe("createTranscriptionProvider", () => {
    ...
    it("throws when creating LiteLLM provider without base URL", () => {
        expect(() =>
            createTranscriptionProvider("litellm", "api-key"),
        ).toThrow("LiteLLM provider requires a base URL");
    });
    ...
    it("throws when creating Azure provider without base URL", () => {
        expect(() =>
            createTranscriptionProvider("azure", "api-key"),
        ).toThrow("Azure provider requires a base URL");
    });
...
describe("inferProviderType", () => {
    ...
    it("infers openai as default", () => {
        ...
        expect(
            inferProviderType("openai", "https://api.openai.com/v1"),
        ).toBe("openai");
    });
});
